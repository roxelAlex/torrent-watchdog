...
Magnet-ссылки поддержаны не полностью: раздача добавится в qBittorrent, но обновления по ней отслеживаться не будут. У magnet нет файла раздачи, поэтому сравнивать новую версию не с чем, а `info_hash` в самой ссылке неизменен — проверка всегда даст «Изменений нет». Для отслеживания нужна тема трекера или прямая ссылка на `.torrent`.
