...
- Ночная проверка падала с `Failed to resolve 'flaresolverr'`. У контейнера задан `container_name`, а при нём алиас по имени сервиса в сети не регистрируется — приложение перестало находить FlareSolverr вовсе, и каждая попытка умирала на DNS, не отправив ни одного запроса наружу. Имя `flaresolverr` теперь прописано в `docker-compose.yml` явно.
- Имя это не косметическое, поэтому чинить нужно именно compose, а не настройки. Приложение отличает наш образ от обычного FlareSolverr ровно по этому хосту и только тогда включает endpoint скачивания. Вписать в настройки имя контейнера — значит сделать вид, что починил: скачивание молча ушло бы на `/v1`, который .torrent отдавать не умеет.

## 0.8.6

- Ночные проверки падали, а та же проверка руками через несколько часов проходила. Cloudflare отказывает окнами в десятки минут, и три попытки с паузой в десять секунд сгорали внутри одного такого окна. Пауза между попытками теперь 1800 секунд, а самих попыток пять — проверка сама ловит момент, когда трекер снова пускает, вместо того чтобы ждать, пока нажмут кнопку.
- Бюджет на прохождение Cloudflare — 120 секунд вместо 60. В плохие ночи только загрузка страницы с проверкой съедала 25–36 секунд из шестидесяти, и попытка падала по таймауту там, где браузеру оставалось несколько секунд; когда всё хорошо, челлендж решается за 12–14 секунд, так что запас ничего не стоит. Клиентские таймауты подняты следом: иначе приложение обрывало бы соединение раньше, чем FlareSolverr сдастся, и настоящая причина не доходила бы до журнала.
- Повтор после входа заново больше не выжидает длинную паузу. Протухшая сессия чинится свежим cookie, пережидать нечего — длинная пауза осталась только для отказов Cloudflare.

## 0.8.5

- Заметки к релизам собираются из `CHANGELOG.md` скриптом `scripts/release_notes.py` — теперь тег не выходит с пустым описанием. Все шесть существующих релизов заполнены задним числом.
- Документация на двух языках: `README.en.md` рядом с русским README, `CHANGELOG.md` по-английски, русский сохранён как `CHANGELOG.ru.md`. Описания на GitHub и Docker Hub — английские.
